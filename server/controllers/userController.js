...
export const signup = async (req, res, next) => {
  ...
  User.find({ username: data.username }, async function(err, docs) {
    if (docs.length) {
      res.status(200).send({ errorMessage: `user ${data.username} already exist` });
    } else {
      const user = new User({
        ...data
      });
...
export const login = (req, res, next) => {
  ...
  User.findOne({ username: username }).exec(function(err, user) {
    if (err) {
      return res.status(200).send({ errorMessage: "login fail, something went wrong" });
    } else if (!user) {
      return res.status(200).send({ errorMessage: `login fail, user ${username} not exist` });
    } else {
